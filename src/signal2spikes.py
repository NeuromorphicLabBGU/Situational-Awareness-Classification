...
from sctn.resonator import resonator as spikes_resonator


def plot_spectrogram(data, fs, fmin, fmax):
    # plot the spectrogram
    plt.figure(figsize=(14, 5))
    Sxx, freqs, bins, im = plt.specgram(data, NFFT=256, Fs=fs, noverlap=128, cmap="jet")
    print(bins)
    plt.ylim(fmin, fmax)
    plt.xlabel("Time (s)")
    ...
    plt.imshow(
        ...
        vmin=0,
        vmax=np.max(Sxx[(freqs >= fmin) & (freqs <= fmax)]),
    )
    plt.colorbar()
    plt.show()
...
def create_datasets(
    time_of_sample_s, overlap, trials, signal_fs, output_path, skip_beginning_s
):
    trials_folder = (
        "../datasets/EEG_data_for_Mental_Attention_State_Detection/EEG_spikes"
    )
    ...
    with tqdm(
        total=len(trials) * 3 * 7 * (14 * 36) * int(1 / time_of_sample_s)
    ) as pbar:
            ...
            for label in os.listdir(f"{trials_folder}/{trial}"):
                for minute in os.listdir(f"{trials_folder}/{trial}/{label}"):
                    ...
                    for band_name, (lf, hf) in bands.items():
                        for ch_name in os.listdir(
                            f"{trials_folder}/{trial}/{label}/{minute}"
                        ):
                            full_minute_spikes_input[f"{ch_name}-{band_name}"] = {}
                            for clk_freq_str in os.listdir(
                                f"{trials_folder}/{trial}/{label}/{minute}/{ch_name}"
                            ):
                                clk_freq = int(clk_freq_str)
                                resonator_freqs = os.listdir(
                                    f"{trials_folder}/{trial}/{label}/{minute}/{ch_name}/{clk_freq}"
                                )
                                resonator_freqs = sorted(
                                    resonator_freqs, key=resonator_fname_to_freq
                                )
                                for f0_str in resonator_freqs:
                                    ...
                                    loaded_spikes = loaded_spikes[
                                        skip_beginning_s * clk_freq :
                                    ]
                                    if int(clk_freq) != signal_fs:
                                        extended_spikes = np.zeros(
                                            signal_fs * (60 - skip_beginning_s),
                                            dtype=np.int8,
                                        )
                                        extended_spikes[
                                            :: int(signal_fs / clk_freq)
                                        ] = loaded_spikes
                    ...
                    for sample_start_index in range(
                        signal_fs,
    ...
    try:
        ...
        if sum(loaded_spikes > 1) > 0:
            ...
            if loaded_spikes[-1] >= 60 * clk_freq:
                raise ValueError(
                    f"it seems that {path}, is not an array of spikes represented by spikes neither by "
                    f"timestamps."
                )
            spikes_encode[loaded_spikes] = 1
            np.savez_compressed(file=path, spikes=spikes_encode)
        return True
...
# first 2 trials for each subject was to understand the experiment
trials = [
    3,
    4,
    5,
        ...
        for ch_i, ch in enumerate(channels):
            ...
            for clk_freq, resonators in clk_resonators.items():
                for f_i, f0 in enumerate(resonators):
                    ...
                    resonator = spikes_resonator(freq0=f0, clk_freq=clk_freq)
                    ...
                    for label, labeled_minutes_range in minutes_range.items():
                        for m in labeled_minutes_range:
                            ...
                            if not is_file_exist(output_folder):
                                data_resampled = resample_signal(
                                    clk_freq,
                                    fs,
                                    ch_data[fs * m * 60 : fs * (m + 1) * 60],
                                )
...
create_datasets(
    time_of_sample_s=0.25,
    overlap=0,
    trials=trials,
    signal_fs=153600,
    output_path="../datasets/EEG_data_for_Mental_Attention_State_Detection/",
    skip_beginning_s=5,
)
